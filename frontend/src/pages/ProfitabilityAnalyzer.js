...
const ProfitabilityAnalyzer = () => {
  ...
  return (
    <div className="p-6" data-testid="profitability-analyzer-page">
      ...
      {showForm && (
        <div className="bg-white border border-[#E2E8F0] shadow-sm p-6 mb-6 animate-scale-in" data-testid="analysis-form">
          ...
          <div className="space-y-5">
            {lineItems.map((item, itemIndex) => {
              ...
              return (
                <div key={itemIndex} className="border border-[#E2E8F0] rounded-lg p-5 bg-[#F8FAFC]">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div className="space-y-1">
                        <Label className="text-xs text-[#64748B]">Revenue (₹)</Label>
                        <Input
                          type="number"
                          value={item.revenue}
                      ...
                      {item.is_subscription && (
                        <div className="space-y-1">
                          <Label className="text-xs text-[#64748B]">Number of Users / Distributors</Label>
                          <Input
                            type="number"
                            ...
                            className="h-10 bg-white"
                          />
                          <p className="text-[10px] text-[#64748B]">Auto-adds L2/L3 + any licenses you select below</p>
                        </div>
                      )}
                    </div>
                    <Button
                    ...
                    <div className="mb-4 bg-amber-50 border border-amber-200 rounded-lg p-3">
                      ...
                      <div className="flex flex-wrap gap-3 mb-3">
                        {Object.keys(rateCard.per_distributor_costs).map((costName) => {
                          ...
                          return (
                            <label key={costName} className="flex items-center gap-1.5 text-xs bg-white px-2.5 py-1.5 rounded border border-[#E2E8F0] cursor-pointer">
                              <input
                                type="checkbox"
                                checked={checked}
                                ...
                                className="accent-purple-600"
                              />
                              {costName} {rate === 0 && <span className="text-[#94A3B8]">(rate pending)</span>}
                            </label>
                          );
                        })}
                      </div>
                      ...
                      {autoBreakdown && (
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
                          <div className="bg-white rounded px-2 py-1.5">
                            <p className="text-[#64748B]">L2 ({autoBreakdown.l2Required.toFixed(3)})</p>
                            <p className="font-semibold">{fmt(autoBreakdown.l2Cost)}</p>
                          </div>
                          <div className="bg-white rounded px-2 py-1.5">
                            <p className="text-[#64748B]">L3 ({autoBreakdown.l3Required.toFixed(3)})</p>
                  ...
                  <div className="pl-4 border-l-4 border-purple-300 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="sm"
                        className="text-purple-600 hover:text-purple-800 h-7"
                      >
                    ...
                    {item.resource_lines.map((rl, lineIndex) => {
                      ...
                      return (
                        <div key={lineIndex} className="bg-white p-2.5 rounded-lg border border-[#E2E8F0] space-y-2">
                          <div className="grid grid-cols-1 md:grid-cols-[2fr_1.2fr_1fr_auto] gap-2 items-end">
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Role</Label>
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Monthly Cost (fixed)</Label>
                              <div className="h-9 flex items-center px-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md text-sm text-[#475569]">
                                {rl.role_name ? fmt(monthlyCost) : '—'}
                              </div>
                            </div>
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Allocation</Label>
                            ...
                            <Button
                              ...
                              variant="ghost"
                              size="sm"
                              className="text-red-600 hover:text-red-800 h-9"
                            >
                              <X size={16} />
                            </Button>
                          </div>

                          <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr] gap-2 items-end pl-1 border-l-2 border-[#E2E8F0]">
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Quantity (optional)</Label>
                              <Input
                                type="number"
                                value={rl.quantity}
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Unit</Label>
                            ...
                            <div className="space-y-1">
                              <Label className="text-[11px] text-[#64748B]">Line Cost</Label>
                              <div className="h-9 flex items-center px-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-md text-xs font-semibold text-[#1E293B]">
                                {fmt(rlCost)}
                              </div>
                            </div>
                          </div>
                    ...
                    {item.resource_lines.length === 0 && (
                      <p className="text-xs text-[#94A3B8] italic">No additional roles added for this line.</p>
                    )}
                  </div>
      ...
      <div className="bg-white border border-[#E2E8F0] shadow-sm">
        <div className="divide-y divide-[#E2E8F0]">
          {analyses.length === 0 ? (
            <div className="p-12 text-center text-[#64748B]" data-testid="no-analyses">
              ...
              <div key={analysis.id} className="p-6" data-testid={`analysis-${analysis.id}`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-[#64748B] mb-3 mt-1">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-[#64748B]">
                        Revenue: <span className="font-semibold text-[#0F172A]">{analysis.total_revenue ? fmt(analysis.total_revenue) : '—'}</span>
                      </span>
                      <span className="text-[#64748B]">
                        Cost: <span className="font-semibold text-[#0F172A]">{fmt(analysis.total_cost)}</span>
                      </span>
                    ...
                    {expandedId === analysis.id && (
                      <div className="mt-4 space-y-2 border-t border-[#E2E8F0] pt-4">
                        {analysis.revenue_line_items.map((li, i) => (
                          <div key={i} className="bg-[#F8FAFC] rounded p-3 text-xs">
                            ...
                            {li.resource_lines.length > 0 && (
                              <div className="mt-2 pl-3 border-l-2 border-purple-200 space-y-0.5">
                                {li.resource_lines.map((rl, j) => (
                                  <div key={j} className="flex justify-between text-[#64748B]">
                                    <span>
                        ...
                        {analysis.notes && (
                          <p className="text-xs text-[#64748B] italic pt-2">Notes: {analysis.notes}</p>
                        )}
                      </div>
                    )}
                  </div>
                  {canEdit(analysis) && (
                    <div className="flex gap-2 flex-shrink-0">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => handleDelete(analysis.id)} className="text-red-600 hover:text-red-800" data-testid={`delete-analysis-${analysis.id}`}>
                        <Trash size={18} />
                      </Button>
                    </div>
                  )}
